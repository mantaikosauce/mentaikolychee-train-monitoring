...
def weather_layers(weather: dict | None) -> list:
    ...
    if "temp_c" in w:
        ...
        layers.append(pdk.Layer("TextLayer", data=t, get_position="[lon, lat]", get_text="text",
                                get_size=13, get_color=[40, 60, 90], get_text_anchor='"start"', character_set="auto",
...
def view_for(points: pd.DataFrame | None, duration_ms: int = 0) -> pdk.ViewState:
    """Camera that fits the given points (lat/lon columns); whole island when empty.
    The view changes instantly (duration_ms is kept for callers and ignored)."""
    ...
    # No transition: pydeck can only send the interpolator as a string, which deck.gl 9
    # rejects ("transitionInterpolator.arePropsEqual is not a function"). That error
    # stops the map's render loop, so it never re-measures and stays at 300x150.
    return pdk.ViewState(**v, pitch=0, bearing=0)
